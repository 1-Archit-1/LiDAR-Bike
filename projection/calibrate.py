...
def project_lidar_to_image_camera2(points, R_vc, T_vc, R02, T02, R_rect, P2)->np.ndarray:
    """Project 3D Lidar points to 2D image"""
    #Transform points to cam0 coordinates
    points_cam0 = (R_vc @ points[:, :3].T).T + T_vc  #(N, 3)

    #transform cam0 to cam2 
    points_cam2 = (R02 @ points_cam0.T).T + T02  #(N, 3)

    #filter out points behind camera where z<0
    points_cam2 = points_cam2[points_cam2[:, 2] > 0]

    #rectification
    points_rect_xyz = (R_rect @ points_cam2.T).T  #(N,3)
    points_rect = np.hstack((points_rect_xyz, np.ones((points_rect_xyz.shape[0], 1)))) #(N,4)

    #Project to 2D 
    points_2d_hom = (P2 @ points_rect.T).T  #(N, 3)
...
def visualize_lidar_on_image(frame_number:str)->str:
    ...
    points_2d = project_lidar_to_image_camera2(points, R_velo_to_cam, T_velo_to_cam, R02, T02, R_rect, P2)
    ...
    os.makedirs("lidar_overlay", exist_ok=True)
    output_path = f"lidar_overlay/output_{frame_number}.png"
